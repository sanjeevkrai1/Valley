...
public final class RadiusUtils {
	/*
	 * The make*Authenticator and encodePapPassword functions are borrowed from
	 * jradius-client, also a sourceforge project and under GPL.
	 */
	/**
	 * This method encodes the plaintext user password according to RFC 2865
	 * 
	 * @param userPass
	 *            java.lang.String the password to encrypt
	 ...
	 */
	public static byte[] encodePapPassword(final MessageDigest md5, final byte[] userPass,
			final byte[] requestAuthenticator, final String sharedSecret) {
	 ...
	 * responses from the RADIUS Authentication process as specified in RFC
	 * 2865. The byte array returned should match exactly the response
	 * authenticator recieved in the response packet.
	 * 
	 * @param code
	 *            byte
	 * @param identifier
	 ...
	 * @param requestAuthenticator
	 *            byte[]
	 * @param responseAttributeBytes
	 *            byte[]
	 * @return byte[]
	 */
	public static byte[] makeRFC2865ResponseAuthenticator(final MessageDigest md5, final String sharedSecret,
			byte code, byte identifier, short length, byte[] requestAuthenticator, byte[] responseAttributeBytes) {
		md5.reset();
	 ...
	 * @return byte[]
	 */
	public static byte[] makeRFC2866RequestAuthenticator(final MessageDigest md5, final String sharedSecret, byte code,
			byte identifier, int length, byte[] requestAttributes) {
		byte[] requestAuthenticator = new byte[16];
